...
bool VoiceLink::Open(const LinkConfig&) {
  return true;
}
...
int main() {
  ...
  steps.push_back(N(Reply()));
  steps.push_back(Delay(20));
  steps.push_back(A(Sound(AudioEventKind::Barge)));
  ...
  require(Run(steps) && state.views.back().state == DeviceUiState::kIdle,
          "text-only done and follow-up timeout");
  steps = {N(Net(LinkEventKind::Online)), A(Sound(AudioEventKind::Wake)), Delay(6001)};
  require(
      Run(steps) && state.packets.empty() && state.views.back().state == DeviceUiState::kIdle,
