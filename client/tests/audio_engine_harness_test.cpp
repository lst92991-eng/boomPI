...
bool WaitForPlaybackDone(const AudioEngine& engine, const std::chrono::milliseconds timeout) {
...
std::vector<std::uint8_t> Pcm24(const std::size_t samples, const std::int16_t value) {
...
bool BeginVoicePlayback(VoiceAudio* const voice, const std::uint32_t generation,
  ...
  for (std::size_t attempt = 0U;
       attempt < 20U && play.wait_for(0ms) != std::future_status::ready; ++attempt) {
    fake::PushCapture(CaptureFrame{});
    std::this_thread::sleep_for(5ms);
  }
...
bool QueueFrame(AudioEngine* const engine, const std::uint64_t sequence,
  ...
  return Check(
      engine->QueueTts24k(bytes.data(), bytes.size(), sequence) == QueueTtsResult::kQueued,
      "QueueTts24k failed");
}
...
bool TestQueueResultsDoNotPolluteLastError() {
  ...
  if (!Check(engine.QueueTts24k(nullptr, 0U, 0U) == QueueTtsResult::kInvalidArgument,
             "invalid PCM did not have a distinct queue result") ||
      !Check(engine.QueueTts24k(frame.data(), frame.size(), 0U) == QueueTtsResult::kNotActive,
             "inactive playback did not have a distinct queue result") ||
      !Check(engine.last_error().empty(), "transient queue rejection polluted last_error") ||
  ...
  if (!Check(engine.QueueTts24k(oversized.data(), oversized.size(), 0U) ==
      ...
      !Check(
          engine.QueueTts24k(frame.data(), frame.size(), 2U) == QueueTtsResult::kDiscontinuous,
          "sequence gap did not have a distinct queue result") ||
      !Check(engine.last_error().empty(), "queue flow control polluted last_error")) {
    return false;
  }
  engine.DropPlayback();
...
bool TestSuccessfulOpenClearsOldError() {
  AudioEngine engine;
  if (!OpenEngine(&engine) ||
      !Check(!engine.Open(AudioEngineConfig{}), "duplicate Open unexpectedly succeeded") ||
      !Check(!engine.last_error().empty(), "duplicate Open did not publish its error")) {
    return false;
  }
  engine.Close();
  return OpenEngine(&engine) &&
         Check(engine.last_error().empty(), "successful reopen retained an old error");
}
...
bool TestSuccessfulPlaybackClearsOldError() {
  ...
  const bool cleared =
      Check(engine.last_error().empty(), "successful playback retained an old error");
  engine.DropPlayback();
  return cleared &&
...
bool TestVoiceAudioPreRollOrder() {
  ...
  if (!Check(
          PollInjected(&voice, admitted, &event) && event.kind == AudioEventKind::SpeechStart,
...
bool TestVoiceAudioFollowUpDropsOldEnd() {
  ...
  for (std::int16_t id = 100; id <= 119; ++id) {
    ...
    if (id != 119 && !Check(!have_event, "follow-up was admitted before 20 new frames")) {
      return false;
    }
    if (id == 119 && !Check(have_event && event.kind == AudioEventKind::SpeechStart,
...
bool DriveConfirmedBarge(VoiceAudio* const voice, AudioEvent* const event) {
  ...
  auto drive = [&](const unsigned count, const bool reference, const bool near_voice) {
    for (unsigned index = 0U; index < count; ++index, ++id) {
      ...
      if (PollInjected(voice, frame, event)) {
        return false;
      }
    }
    return true;
...
bool TestVoiceAudioBargeAndDropLifecycle() {
  ...
  if (!Check(DriveConfirmedBarge(&voice, &event),
      ...
      !Check(event.generation == 7U, "Barge was not bound to the interrupted generation")) {
    return false;
  }
