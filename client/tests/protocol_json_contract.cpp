...
void SharedFixtures(const char* path) {
  using namespace boompi::voice_net;
  ...
  for (const auto* item = controls->child; item; item = item->next) {
    ...
    // hello/start/end/cancel 属于上行控制，由真实 TLS 回环测试检查；下行解码器应拒绝上行类型。
    if (type == "hello" || type == "start" || type == "end" || type == "cancel") {
      continue;
    }
...
bool Accepted(const std::string& json) {
  try {
    (void)boompi::voice_net::detail::DecodeText(json);
    return true;
  } catch (...) {
    return false;
  }
