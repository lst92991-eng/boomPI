...
std::uint32_t Number(const cJSON* object, const char* key) {
  const auto* item = cJSON_GetObjectItemCaseSensitive(object, key);
  Check(cJSON_IsNumber(item) && std::isfinite(item->valuedouble) &&
...
void SharedFixtures(const char* path) {
  ...
  for (const auto* item = controls->child; item; item = item->next) {
    ...
    if (type == "hello" || type == "stop") continue;
    const auto name = Field(item, "name");
    const auto event = detail::DecodeText(Field(item, "wire_text"));
    if (type == "ready") {
      Check(event.kind == LinkEventKind::Online && event.generation == 0, name + ": ready mismatch");
    } else {
      Check(event.generation == Number(expected, "generation"), name + ": generation mismatch");
      if (type == "text")
        Check(event.kind == LinkEventKind::Text && event.text == Field(expected, "text"), name + ": text mismatch");
      else if (type == "done") Check(event.kind == LinkEventKind::Done, name + ": done mismatch");
      else if (type == "error")
        Check(event.kind == LinkEventKind::Error && event.code == Field(expected, "code"), name + ": error mismatch");
      else throw std::runtime_error(name + ": unsupported fixture control");
    }
    ++control_count;
  }
  for (const auto* item = audio->child; item; item = item->next) {
    ...
    Check(wire == HexBytes(Field(item, "header_hex")) + payload, name + ": inconsistent fixture bytes");
    ...
    if (direction == "uplink") {
      ...
      for (std::size_t i = 0; i < pcm.size(); ++i) {
        const auto value = static_cast<unsigned char>(payload[2 * i]) |
            (static_cast<unsigned>(static_cast<unsigned char>(payload[2 * i + 1])) << 8U);
        pcm[i] = static_cast<std::int16_t>(value < 32768 ? static_cast<int>(value) : static_cast<int>(value) - 65536);
      }
      ...
      Check(event.kind == LinkEventKind::Audio && event.generation == generation && event.sequence == sequence &&
            event.start == ((flags & 1U) != 0) && event.end == ((flags & 2U) != 0) &&
            event.audio_size == payload.size() &&
            std::string(reinterpret_cast<const char*>(event.audio.data()), event.audio_size) == payload,
            name + ": decoded audio differs from shared golden fields");
      ++downlink_count;
    }
  }
  Check(control_count >= 4 && uplink_count >= 2 && downlink_count >= 3, "shared fixture coverage is incomplete");
  std::cout << "v2 shared fixture: " << control_count << " controls, " << uplink_count << " uplink, "
            << downlink_count << " downlink passed\n";
}
...
int main(int argc, char** argv) {
  if (argc > 2) { std::cerr << "usage: protocol-json-test [shared-fixture.json]\n"; return 1; }
  ...
  const std::vector<std::pair<std::string, bool>> cases{
    ...
    {ready + "{}", false}, {invalid_utf8, false}, {embedded_nul, false},
