...
constexpr unsigned kAecWarmupFrames = static_cast<unsigned>(
    VoiceFrameContract::FramesForMs(600U));
constexpr unsigned kAecTailFrames = static_cast<unsigned>(
...
struct AudioBackend::Impl final {
  ...
  bool UpdateNearVoice(CaptureFrame* frame) noexcept {
    ...
    if (playback_ended.exchange(false, std::memory_order_acq_rel)) {
      ...
      if (pcm.playback_interrupted()) aec_tail_remaining = 0U;
...
bool AudioBackend::Open(const AudioEngineConfig& config) noexcept {
  ...
  impl_->ClearError();
  if (!impl_->pcm.Open(config.capture_pcm, config.playback_pcm)) return false;
...
bool AudioBackend::ReadCapture20ms(bool* const discontinuity) noexcept {
  ...
  if (!impl_->pcm.ReadCapture20ms(impl_->capture48.data(), discontinuity)) return false;
...
bool AudioBackend::PreparePlayback() noexcept {
  if (impl_ == nullptr || !impl_->open || !impl_->pcm.PreparePlayback()) return false;
...
bool AudioBackend::Render20ms(const std::int16_t* const pcm24, std::size_t samples, float gain) noexcept {
  ...
  if (peak != 0) {
    ...
    impl_->playback_output_audible.store(gain > 0.0F,
...
void AudioBackend::InterruptCapture() noexcept {
  if (impl_ != nullptr) impl_->pcm.InterruptCapture();
}
...
std::string AudioBackend::last_error() const {
  ...
  return impl_->error[0] != '\0' ? std::string(impl_->error.data()) : impl_->pcm.last_error();
}
void AudioBackend::Close() noexcept {
  ...
  impl_->playback_session_active = impl_->aec_warmup_armed =
      impl_->silent_playback_bypass = false;
