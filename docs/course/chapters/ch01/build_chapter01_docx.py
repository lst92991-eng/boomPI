...
SOURCE = HERE / "chapter01.md"
OUTPUT = HERE / "第01章_开发板简介_审阅稿_v0.8.docx"
...
def configure_header_footer(doc: Document) -> None:
    ...
    run = fp.add_run("第1章 开发板简介 · 独立审阅稿  |  ")
    set_font(run, size=9, color=GRAY)
    add_page_field(fp)
...
def parse_markdown(doc: Document, source: str):
    ...
    while i < len(lines):
        ...
        if stripped.startswith("# "):
            ...
            heading = re.sub(r"^第\s*1\s*章\s*", "", stripped[2:])
            add_inline_text(p, heading, size=15)
        elif stripped.startswith("## "):
            p = doc.add_paragraph(style=H2)
            heading = re.sub(r"^1\.\d+\s*", "", stripped[3:])
...
def main():
    ...
    doc.core_properties.title = "第1章 开发板简介"
    doc.core_properties.subject = "boomPI 独立章节审阅稿"
    doc.core_properties.author = "尚硅谷嵌入式项目"
    doc.core_properties.keywords = "boomPI, RV1106G3, Linux, 开发板, 原理图"
    add_cover(doc)
