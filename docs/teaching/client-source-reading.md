...
3. `client/src/audio/audio_capture.cpp`：真实采集线程中的ALSA→转换→3A→wake→vad。
...
UI/触摸/摄像头继续沿用原有模块；服务端保持Key-only配套黑箱。旧结构与旧API由Git历史保存，不保留可调用兼容层。协议见[BPV3](../../protocol/protocol-v3.md)。
