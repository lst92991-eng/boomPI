...
## 2.1 v1.0.0 板端实现边界（2026-08-04，优先于后文路线图）

`v1.0.0` 是第一版真板人工验收后的不可变教学基线。后续功能、性能或声学优化必须从该标签或
合并后的 `main` 开始，保留同条件前后证据，并重新通过 host、严格交叉构建和目标板闭环；
不得移动标签或用一次主观试听覆盖量化边界。

现役 `boompi-client` 的音频主线包含 15 个生产 C/C++ 文件、3078 ELOC，其中
Rockchip/Snowboy vendor 集成 280 ELOC，音频产品逻辑 2798 ELOC。此次增长主要来自把状态转换、
