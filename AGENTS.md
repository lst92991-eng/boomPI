...
- 板端语音核心生产 C/C++（含 Rockchip/Snowboy 适配与网络启动，不含 UI 显示层）硬上限为 **2666 ELOC**（按修正统计口径后的实测值重定基线，含 Phase 2 结构性重构与 Phase 3 对标业界的正确性修复净开销；不为数字压行）；
...
- 增加生产 C/C++ 前必须先删减或用实测证明必要性，语音核心（含 vendor 与网络启动）不得超过
  2666 ELOC；UI 显示层单独公开行数、按需增长但需评审。辅助运维与配网脚本单独计数；未来 supervisor 不得挤入当前客户端预算。
