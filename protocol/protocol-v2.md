# boomPI v2 教学协议（16 kHz 配套版本）

状态：客户端与服务端共同实施的唯一协议。沿用 BPV2 头部与轮次语义，握手新增必需的采样率声明；与旧 24 kHz 版本不兼容，必须成套升级。旧版本由 Git 历史保存，不提供双栈回退。

## 固定连接

- WSS `/ws`，TCP 17806，TLS/SPKI 保持现有可信课堂局域网部署方式。
- UDP 17807 请求 `BOOMPI_DISCOVER_V2`，回复 `BOOMPI_SERVER_V2 <port> <spki>`。
- 客户端首个 JSON：`{"type":"hello","device_id":"<canonical uuid>","token":"<existing teaching token>","sample_rate":16000}`。
- 服务端验证后回复 `{"type":"ready","sample_rate":16000}`。双方均严格要求 `sample_rate` 为整数 16000；缺失或其他值在传音频前拒绝。设备身份只在握手发送一次。
...
- 下行固定 16 kHz，1..320 个 sample；非末帧必须为 640 字节。
- 20 ms 是当前网络组帧单位，不约束 vendor 算法块长或声卡 period；底层仍根据实际 SDK 和硬件适配。
...
- DONE 不代表声卡播完。客户端必须等播放队列与 ALSA 尾播完成，再进入追问窗口。
...
上传 END 仅结束输入方向，等待回答、流式播放和尾播期间 STOP 仍有效。generation 隔离在途旧回答，sequence 检查应用内丢帧；WebSocket 有序不能替代这两项职责。
